...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  const todayStats = useMemo(() => {
    // Lọc lịch trực của ngày hôm nay
    const branchAssignments = assignments.filter(a => 
      a.date === todayDate && 
      (user.role === Role.ADMIN || a.branchId === user.branchId)
    );
    
    const totalScheduled = branchAssignments.length;
    
    // Lọc logs của ngày hôm nay
    const todayLogs = logs.filter(l => 
      l.date === todayDate && 
      (user.role === Role.ADMIN || l.branchId === user.branchId)
    );

    const checkIns = todayLogs.filter(l => !!l.checkInTime).length;
    
    // Những ca được coi là hoàn tất: Đã có checkOutTime hoặc status là COMPLETED
    const closedShifts = todayLogs.filter(l => !!l.checkOutTime || l.status === 'COMPLETED').length;
    
    // Những ca đợi duyệt: Đã hoàn tất nhưng chưa isConfirmed
    const pendingApprovals = todayLogs.filter(l => 
      (!!l.checkOutTime || l.status === 'COMPLETED') && !l.isConfirmed
    ).length;
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 animate-in pb-32">
      ...
      {/* Modal form notification/regulation omitted for brevity as it hasn't changed */}
    </div>
  );
};
